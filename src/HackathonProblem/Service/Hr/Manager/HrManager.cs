...
public class HrManager(ITeamBuildingStrategy strategy) : IHrManager
{
    ...
    )
    {
        foreach (var teamLeadWishlist in teamLeadsWishlists)
        {
            if (teamLeadWishlist.DesiredEmployees.Length != juniors.Count())
            {
                throw new ArgumentException(
                    "The number of the team lead's preferences does not match the number of juniors"
                    + $" (owner ID: {teamLeadWishlist.EmployeeId})"
                );
            }
        }
        ...
        foreach (var juniorWishlist in juniorsWishlists)
        {
            ...
            {
                throw new ArgumentException(
                    "The number of junior's preferences does not match the number of team leads"
                    + $" (owner ID: {juniorWishlist.EmployeeId})"
                );
            }
        }
